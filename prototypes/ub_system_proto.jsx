...
export default function App(){
  ...
  useEffect(function(){
    ...
    if(ctxRoll===0){edges=assignCtx(edges)}
    ...
    for(var ti=0;ti<trimmed.length;ti++){var te=trimmed[ti];
      ...
      var pl=pick(te);if(te.length<Math.min.apply(null,pl.lens)){te.length=0;te.bufs=null;te.secs=[];te.removed=true}
    }
    ...
    var secFire=[];
    var roadBuf=[]; // дорожный буфер 14м ВО ВСЕ СТОРОНЫ от группы секций
    var RB=14; // размер дорожного буфера
    for(var wi=0;wi<withSecs.length;wi++){var we=withSecs[wi];
      ...
      for(var gi=0;gi<groups.length;gi++){var g=groups[gi];
        ...
        // Fire buffer: fb от длинных сторон (без расширения торцов)
        secFire.push([vAdd(f[0],vSc(nod,fb)),vAdd(la[1],vSc(nod,fb)),vAdd(la[2],vSc(od,fb)),vAdd(f[3],vSc(od,fb))]);
        // Road buffer: RB от ВСЕХ сторон (включая торцы)
        var axDir=vNorm(vSub(la[1],f[0]));
        var negAx=vSc(axDir,-1);
        roadBuf.push([
          vAdd(vAdd(f[0],vSc(nod,RB)),vSc(negAx,RB)),   // start-outer - RB по оси назад, RB наружу
          vAdd(vAdd(la[1],vSc(nod,RB)),vSc(axDir,RB)),   // end-outer + RB по оси вперёд, RB наружу
          vAdd(vAdd(la[2],vSc(od,RB)),vSc(axDir,RB)),    // end-inner + RB по оси вперёд, RB внутрь
          vAdd(vAdd(f[3],vSc(od,RB)),vSc(negAx,RB))      // start-inner - RB по оси назад, RB внутрь
        ]);
      }
    }
  ...
  },[poly,gridStep,sw,fb,eb,ib,latL,lonL,tg,ctxRoll]);
  ...
  var draw=useCallback(function(){
    ...
    if(vSec)for(var sei=0;sei<edges.length;sei++){var se=edges[sei];if(!se.secs||!se.secs.length)continue;var ci2=CC[se.context]||CC[2];
      for(var sj=0;sj<se.secs.length;sj++){var sec=se.secs[sj];var r=sec.rect.map(ws);
        ...
        else{c.fillStyle=sec.BAD?"rgba(255,0,0,0.6)":ci2.f;c.fill();c.strokeStyle=sec.BAD?"#ff0000":"#1d3557";c.lineWidth=sec.BAD?3:1.5;c.stroke();
          if(vLab){var cx=(r[0].x+r[1].x+r[2].x+r[3].x)/4,cy=(r[0].y+r[1].y+r[2].y+r[3].y)/4;var tx=sec.BAD?"!"+sec.length.toFixed(1):sec.length.toFixed(0)+"м";c.font="bold 10px monospace";c.textAlign="center";c.textBaseline="middle";var tw=c.measureText(tx).width+6;c.fillStyle=sec.BAD?"rgba(200,0,0,0.9)":"rgba(29,53,87,0.85)";c.fillRect(cx-tw/2,cy-7,tw,14);c.fillStyle="#fff";c.fillText(tx,cx,cy)}}}}
  ...
  },[mode,dPts,mw,comp,vGrid,vBuf,vSec,vAx,vLab,vGhost,vYard,vRoad,vGraph,fb,eb,ib,ct]);
  ...
  var loadP=function(n){var pts=PR[n];if(!pts)return;var p=makeCCW(pts.slice());setPoly(p);setDPts([]);setMode("view");setCtxRoll(0);camRef.current=fitCam(p,CW,CH);setCt(function(t){return t+1})};
  var reset=function(){setPoly(null);setDPts([]);setMode("draw");setComp(null);setCtxRoll(0);camRef.current=makeCam(100,80,3.5);setCt(function(t){return t+1})};

  var stats=comp?function(){var ts=0,tl=0,inv=0,lS=0,mS=0,d={};
    for(var i=0;i<comp.edges.length;i++){var e=comp.edges[i];if(!e.secs)continue;for(var j=0;j<e.secs.length;j++){var s=e.secs[j];if(s.isGap)continue;ts++;tl+=s.length;d[s.length]=(d[s.length]||0)+1;if(s.BAD)inv++;if(e.secTag==="ш")lS++;else mS++}}
    return{ts:ts,tl:tl,d:d,inv:inv,lS:lS,mS:mS,ax:comp.edges.filter(function(e){return e.secs&&e.secs.length>0}).length,tr:comp.edges.filter(function(e){return e.trimmed&&!e.removed}).length,rm:comp.edges.filter(function(e){return e.removed}).length,cn:comp.connectors?comp.connectors.length:0}}():null;
    ...
    <div style={{width:"100%",minHeight:"100vh",background:"#f4f5f7",fontFamily:"'JetBrains Mono','Fira Code',monospace",color:"#264653",display:"flex",flexDirection:"column"}}>
      <div style={{padding:"10px 16px",background:"linear-gradient(135deg,#1d3557,#264653)",color:"#f1faee",display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,flexWrap:"wrap"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <span style={{fontSize:10,opacity:0.6,borderLeft:"1px solid rgba(255,255,255,0.2)",paddingLeft:8}}>{"v5 · insol="+ib+"м"}</span>
        </div>
        <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
          ...
          <button onClick={function(){setCtxRoll(function(r){return r+1})}} style={Object.assign({},bS,{borderColor:"#f4a261",background:"rgba(244,162,97,0.2)",fontWeight:700})}>{"Перебор #"+ctxRoll}</button>
      ...
      <div style={{display:"flex",flex:1,minHeight:0}}>
        <div style={{width:240,minWidth:240,padding:"10px 12px",background:"#fff",borderRight:"1px solid #e0e0e0",overflowY:"auto",fontSize:11}}>
          ...
          {stats && <div style={{marginTop:6,padding:5,background:"#eef2f7",borderRadius:4}}>
            ...
            <div>{"Секций: "+stats.ts+" ([ш]"+stats.lS+" [м]"+stats.mS+")"}</div>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:12}}>
          ...
          {mode==="view" && <div style={{marginTop:5,fontSize:10,color:"#999"}}>{"Ш/М = ориентация · [ш]="+latL+" · [м]="+lonL}</div>}
        </div>
      </div>
    </div>
  );
