...
export default function App(){
  ...
  useEffect(function(){
    ...
    var secFire=[];
    var roadBuf=[];
    var trashInner=[]; // 20м буфер (ближе нельзя)
    var trashOuter=[]; // 100м буфер (дальше нельзя)
    var RB=14;
    ...
    for(var wi=0;wi<withSecs.length;wi++){var we=withSecs[wi];
      ...
      for(var gi=0;gi<groups.length;gi++){var g=groups[gi];
        ...
        roadBuf.push([
          vAdd(vAdd(f[0],vSc(nod,RB)),vSc(negAx,RB)),
          vAdd(vAdd(la[1],vSc(nod,RB)),vSc(axDir,RB)),
          vAdd(vAdd(la[2],vSc(od,RB)),vSc(axDir,RB)),
          vAdd(vAdd(f[3],vSc(od,RB)),vSc(negAx,RB))
        ]);
        // Мусорные буферы: от ВСЕХ сторон группы секций
        trashInner.push([
          vAdd(vAdd(f[0],vSc(nod,TB_IN)),vSc(negAx,TB_IN)),
          vAdd(vAdd(la[1],vSc(nod,TB_IN)),vSc(axDir,TB_IN)),
          vAdd(vAdd(la[2],vSc(od,TB_IN)),vSc(axDir,TB_IN)),
          vAdd(vAdd(f[3],vSc(od,TB_IN)),vSc(negAx,TB_IN))
        ]);
        trashOuter.push([
          vAdd(vAdd(f[0],vSc(nod,TB_OUT)),vSc(negAx,TB_OUT)),
          vAdd(vAdd(la[1],vSc(nod,TB_OUT)),vSc(axDir,TB_OUT)),
          vAdd(vAdd(la[2],vSc(od,TB_OUT)),vSc(axDir,TB_OUT)),
          vAdd(vAdd(f[3],vSc(od,TB_OUT)),vSc(negAx,TB_OUT))
        ]);
      }
    }
  ...
  },[mode,dPts,mw,comp,vGrid,vBuf,vSec,vAx,vLab,vGhost,vYard,vRoad,vGraph,vTrash,fb,eb,ib,ct]);
